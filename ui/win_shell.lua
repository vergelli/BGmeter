...
local function build_settings()
    ...
    local emblem = P.icon(win, K.LOGO)
    emblem:SetDimensions(20, 20)
    emblem:SetAnchor(TOPLEFT, win, TOPLEFT, 16, 13)
    local title = P.label(win, S.FONT.title, K.COLOR.text)
    title:SetText(K.TITLE .. "  ·  Settings")
...
local function apply_visibility()
    ...
    if not want then
        W._chart_hover_stop()
        hide_medal_card()
        if BGMeter.UI.map then BGMeter.UI.map.on_report_hidden(true) end
...
local function after_hide(was_visible)
    ...
    W._chart_hover_stop()
    hide_medal_card()
    if BGMeter.UI.map then BGMeter.UI.map.on_report_hidden(false) end
    if was_visible then
        Sound.play("close")
