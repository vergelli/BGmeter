...
local function build_battle(win)
    ...
    local function glyph(tex, x)
        local glow = P.icon(b.bal, "bgmeter/assets/glow.dds")
        glow:SetDimensions(BAL_ICON * 2, BAL_ICON * 2)
        glow:SetAnchor(TOPLEFT, b.bal, TOPLEFT, x - BAL_ICON / 2, BAL_Y - BAL_ICON / 2)
        if glow.SetBlendMode then glow:SetBlendMode(TEX_BLEND_MODE_ADD) end
        glow:SetDrawLevel(1)
    ...
    b.balIcon, b.balGlow = glyph(Icons.BALANCE, 6)
    b.balScore = value(b.balIcon, 30)
    b.balDecBar = P.rect(b.bal, { 1, 1, 1, 0.10 })
    b.balDecBar:SetDimensions(64, 4)
    b.balDecBar:SetAnchor(LEFT, b.balScore, RIGHT, 6, 0)
    b.balDecFill = P.rect(b.bal, { 1, 1, 1, 0.35 })
    b.balDecFill:SetAnchor(TOPLEFT, b.balDecBar, TOPLEFT, 0, 0)
    b.balDecFill:SetDimensions(0, 4)
    b.balDecDot = P.rect(b.bal, { 1, 1, 1, 0.9 })
    b.balDecDot:SetDimensions(3, 10)
    b.balBaseIcon, b.balBaseGlow = glyph(Icons.CAMP, 150)
    b.balBase = value(b.balBaseIcon, 44)
    b.balStopIcon, b.balStopGlow = glyph(Icons.SHEATHED, 236)
