...
- Map: the recorder samples your position, your teammates' and every objective pin every 3 s, keeps the arena's tile art with the match, and stamps your kills and deaths with where they happened. A map button in the report header opens a panel under the report: the real arena map with your team's presence heat, your path in gold, teammates' paths on request, skulls where you died and where you got kills, and flags, relics and balls where they were at the scrubbed second. Your own path is sampled every second and drawn as a smooth curve. A slider under the map moves through the match, and hovering the timeline chart moves it too. Layers toggle from the panel. The panel docks under the report; drag it away to float it, drag it back to dock; it resizes, and the map grows with it. A MAP button next to the match name opens it, dim when the match predates 0.4.0. About shows a third storage bar for the map data.
