...
- About drawer at the bottom edge of the Registry (overview glyph): version, author, credits (unit220, TattoozNbooZ), and the caps for matches and familiar faces, with a STORAGE panel of two bars, matches and faces, reading bytes used over bytes available. Available is not a game limit: it is the size the store would reach at the addon's own caps, saved matches included, estimated the way the game writes saved variables.
